...
const KEY_PONTOS_TOTAL = 'dds-contexto-pontos-total'; // pontuação total acumulada
...
function loadPontosTotal(){ try { return parseInt(localStorage.getItem(KEY_PONTOS_TOTAL)) || 0; } catch { return 0; } }
function savePontosTotal(pontos){ localStorage.setItem(KEY_PONTOS_TOTAL, pontos.toString()); }
...
// ===== Renderizar histórico persistente =====
function renderHistory(){
  ...
  hist.forEach(entry => {
    const li = document.createElement('li');
    const prox = entry.prox;
    li.innerHTML = `<span class="badge ${prox.cls}">${prox.icon} ${prox.dist===Infinity?'—':prox.dist}</span> <strong>${entry.g}</strong> <span class="muted small">(${prox.tip} — ${prox.pct}%)</span>`;
...
async function main(){
  ...
  // Carrega pontos totais salvos
  state.pontosTotal = loadPontosTotal();
  ...
  on($('#iniciar'), 'click', ()=>{
    ...
    state.idxWord = 0; state.hintStep=0; 
    // Carrega pontos totais do localStorage
    state.pontosTotal = loadPontosTotal(); 
    $('#pontosTotal').textContent = state.pontosTotal;
    novaRodada();
  });
  ...
  on($('#enviar'), 'click', async ()=>{
    ...
    if (guess === state.answer){
	      $('#feedback').textContent = '🎉 Parabéns! Próxima palavra…'; 
	      state.pontosTotal += state.pontosRodada; 
	      $('#pontosTotal').textContent = state.pontosTotal; 
	      // Salva pontos totais no localStorage
	      savePontosTotal(state.pontosTotal);
	      const w=state.answer; setProximity(100);
